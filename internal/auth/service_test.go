...
func TestSessionLifecycleAndRoleBoundaries(t *testing.T) {
	...
	session, err := service.Login(context.Background(), "regulator", "regulator-demo")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := service.RequireRole(context.Background(), session.ID, RoleProsecutor); !errors.Is(err, ErrForbidden) {
		t.Fatalf("expected role denial, got %v", err)
	}
	if _, err := service.RequireRole(context.Background(), session.ID, RoleRegulator); err != nil {
		t.Fatalf("expected regulator access, got %v", err)
	}
...
func TestExpiredSessionIsRejected(t *testing.T) {
	...
	session, err := service.Login(context.Background(), "operator", "operator-demo")
	if err != nil {
		t.Fatal(err)
	}
